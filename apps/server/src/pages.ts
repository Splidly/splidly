...
export function termsPage(env: Env): string {
  ...
  return documentLayout(
    ...
    `<h1>Terms of Service</h1>
     ...
     <p>You can delete your account from Profile or the public account-deletion page. Identifying account and profile data is removed. Shared groups, memberships, pictures, expenses, settlements, descriptions, notes, amounts, dates, currencies, payers, splits, and audit history remain unchanged so deletion cannot corrupt another participant's ledger. Your identity is displayed as “Deleted user.”</p>
...
export function privacyPage(env: Env): string {
  ...
  const backupRecipient = env.BACKUPS_ENABLED
    ? `<li><strong>Backups:</strong> encrypted database recovery copies are stored on the same netcup production server and automatically deleted after seven days.</li>`
    : "";
  const backupRetention = env.BACKUPS_ENABLED
    ? " Protected database backups use a seven-day retention schedule, so deleted identity data may remain in a backup until that backup expires."
    : " Splidly does not currently create database backups.";
  return documentLayout(
    "Privacy · Splidly",
    `<h1>Privacy notice</h1>
     <p><small>Effective September 3, 2026</small></p>
     ...
     <ul>
       ...
       <li><strong>Notification data:</strong> a random installation identifier, Apple Push Notification service device token, delivery status, and notification content about shared group activity, including the actor, group, expense description, amount, and the recipient's involvement.</li>
     ...
     <ul>
       ...
       <li><strong>Shared-ledger integrity after deletion - Article 6(1)(f) GDPR:</strong> shared records are retained unchanged for the legitimate interests of the other participants and Splidly in preserving correct shared balances and resolving disputes. Direct account and profile identifiers are removed and the participant is displayed as “Deleted user.”</li>
     ...
     <p>The automatic in-app deletion described above is separate from a GDPR rights request. Requests for further erasure or an objection to continued processing are assessed individually, including whether legal obligations, overriding legitimate grounds, or the rights of other participants require continued retention.</p>
     ...
     <h2>Security</h2>
     <p>Splidly uses HTTPS in transit, protected device storage for session credentials, and access controls for servers and databases. When backups are enabled, they are encrypted and access-controlled. No internet service can guarantee absolute security.</p>
  ...
  if (signedIn) {
    ...
    return documentLayout(
      ...
      `<h1>Delete your account</h1>
       <p>Your identity, sign-in connections, sessions, invitations, notifications, and private cached data will be removed immediately. Shared groups and financial records remain unchanged so every participant's balance stays correct. Your name is shown as “Deleted user.”</p>
